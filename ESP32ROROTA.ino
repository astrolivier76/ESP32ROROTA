...
void codeNet(void *p) {
  ...
  serverWeb.on("/update", HTTP_GET, []() {
    ...
    String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1'><style>body{background:#111;color:#eee;text-align:center;padding:30px;font-family:sans-serif;}form{background:#222;padding:30px;border-radius:10px;display:inline-block;border:1px solid #444;}input[type='file']{margin:20px 0;font-size:16px;}.btn{padding:15px 30px;background:#2f855a;color:#fff;border:none;border-radius:8px;font-weight:bold;cursor:pointer;font-size:16px;}</style></head><body><h2>Mise a jour Firmware (OTA)</h2><form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update' accept='.bin'><br><input type='submit' class='btn' value='Flasher l ESP32'></form><br><br><a href='/' style='color:#888;text-decoration:none;'>Retour accueil</a></body></html>";
    serverWeb.send(200, "text/html", html);
  });
